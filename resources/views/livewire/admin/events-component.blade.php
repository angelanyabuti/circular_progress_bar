                ...
                <div class="table-responsive">
                    <table class="table">
                        ...
                        @foreach($events as $event)
                            <tr>
                                <td>
                                    <img src="{{ $event->image }}" class="mr-75" height="20" width="20" alt="{{ $event -> name }}">
                                </td>
                                <td>{{ $event -> name }} </td>
                                <td>{{ $event -> description }}</td>
                                <td>{{ $event -> getEventSubscriptions() }}</td>
                                <td>{{ $event -> updated_at }}</td>
                                <td>
                                    <div class="custom-control custom-switch">
                                        <input wire:click="toggleActive({{ $event }})" @if($event ->status == 1) checked @endif type="checkbox" class="custom-control-input" id="{{$event ->id}}">
                                        <label class="custom-control-label" for="{{$event ->id}}">
...
{{--                                            <a class="dropdown-item" href="{{ route('event.subs', $event->id) }}">--}}
{{--                                                <i data-feather="eye" class="mr-50"></i>--}}
{{--                                                <span>Edit</span>--}}
{{--                                            </a>--}}
                                            <a class="dropdown-item" href="javascript:void(0);" data-toggle="modal" data-target="#exampleModal" wire:click="edit({{ $event->id }})" >
                                                <i data-feather="edit-2" class="mr-50"></i>
                                                <span>Edit</span>
                                            </a>
                                            <a class="dropdown-item" href="javascript:void(0);" wire:click="setActive({{ $event->id }})" onclick="del()">
        ...
        <div class="modal-dialog">
            <form class="add-new-user modal-content pt-0">
                ...
                <div class="modal-header mb-1">
                    <h5 class="modal-title" id="exampleModalLabel">Create Special Event</h5>
                </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 col-6 profile-latest-img">
                                <a href="javascript:void(0)">
                                    <img src="{{ asset('storage/'.$active ->image) }}" class="img-fluid rounded" alt="avatar img">
                                </a>
                            </div>
                        </div>
                    @endif
